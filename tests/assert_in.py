...
from myassert.assertions import assert_in
...
def test_member_in_container_passes(member, container):
...
def test_member_not_in_container_fails(member, container):
    with pytest.raises(AssertionError):
        assert_in(member, container)


def test_call_from_class_passes():
    AllAssertions.assert_in(full_string[:3], full_string)


def test_call_from_class_fails():
    with pytest.raises(AssertionError):
        AllAssertions.assert_in(150009, [1, 2, 3, 4, 5, 6, 7, 8, 9])
